...
public class OobeActivity extends Activity implements OnItemClickListener,
    ...
    private int mCurrentLanguage = -1; // 0: chinese simple , 1: english ,2:
                                       // chinese taiwan
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        oobe_mEthernetManager = (EthernetManager) mContext
                .getSystemService("ethernet");
        oobe_mWifiManager = (WifiManager) mContext
    ...
    private void initNetView() {
        ...
        if (productModel.contains("g35")) {
            ...
            oobe_no_network.setText(mContext.getResources().getString(
        ...
        oobe_password_editview.setInputType(InputType.TYPE_CLASS_TEXT
    ...
    private void wifiResume() {
        if (getEthCheckBoxState()) {
            if (isEthDeviceAdded()) {
                // removed by vim, wifi is open by default
    ...
    private void enableEthernetView(boolean able) {
        if (able) {
            ...
            // remove by vim, keep wifi enable
            // oobe_mWifiManager.setWifiEnabled(false);
            // end
    ...
    private void updateEthCheckBoxUI() {
        if (getEthCheckBoxState()) {
            oobe_select_ethernet.setCompoundDrawablesWithIntrinsicBounds(
    ...
    private void upDateWifiCheckBoxUI() {
        if (getWifiCheckBoxState()) {
            oobe_select_wifi.setCompoundDrawablesWithIntrinsicBounds(
                    R.drawable.ic_checked, 0, 0, 0);
        } else {
            oobe_select_wifi.setCompoundDrawablesWithIntrinsicBounds(
                    R.drawable.ic_uncheck, 0, 0, 0);
    ...
    private void showWifiConnectedView() {
        ...
        if (mWifiinfo != null) {
            ...
            oobe_mAccessPointListAdapter.setCurrentConnectItemSSID(mWifiinfo
                    .getSSID());
        }
    ...
    private void selectLanguageByID(int id, boolean hasFocus) {
        Log.d(TAG, "===== selectLanguageByID() , hasFocus :" + hasFocus
                + " , mCurrentLanguage :" + mCurrentLanguage);
        if (id == R.id.select_cn) {
            Log.d(TAG, "===== select_cn,hasFocus: " + hasFocus);
            if (mCurrentLanguage == 0) {
                if (hasFocus) {
                    select_cn
                            .setBackgroundResource(R.drawable.language_selected);
                } else {
                    select_cn
                            .setBackgroundResource(R.drawable.language_current);
                }
            } else if (mCurrentLanguage == 1) {
                if (hasFocus) {
                    select_cn
                            .setBackgroundResource(R.drawable.language_seclect_focused);
                } else {
    ...
    private void closeScreenAdjustLayout() {
         ...
         * 
         * @Override public void run() { Log.d(TAG,"===== save position now");
         * mScreenPositionManager.savePostion(); } }); t.start();
         */
    ...
    private void startTimer() {
        if (task == null) {
            task = new TimerTask() {
                ...
                @Override
                public void run() {
                    Message msg = oobe_mAccessPointRefreshHander
                            .obtainMessage();
    ...
    private void setCurrentViewSelected(int id) {
        if (id == R.id.top_welcome) {
            ...
            top_welcome
                    .setBackgroundResource(R.drawable.image_top_welcome_focus);
            top_language_layout
                    .setBackgroundResource(R.drawable.image_top_language);
            top_language.setBackgroundResource(Color.TRANSPARENT);
            top_network_layout
                    .setBackgroundResource(R.drawable.image_top_network);
            top_network.setBackgroundResource(Color.TRANSPARENT);
            top_screen_layout
                    .setBackgroundResource(R.drawable.image_top_screen);
            top_screenadjust.setBackgroundResource(Color.TRANSPARENT);
        } else if (id == R.id.top_language) {
            top_welcome_layout
                    .setBackgroundResource(R.drawable.image_top_welcome);
            top_welcome.setBackgroundResource(Color.TRANSPARENT);
            top_language_layout.setBackgroundResource(Color.TRANSPARENT);
            top_language
                    .setBackgroundResource(R.drawable.image_top_language_focus);
            top_network_layout
                    .setBackgroundResource(R.drawable.image_top_network);
            top_network.setBackgroundResource(Color.TRANSPARENT);
            top_screen_layout
            ...
            top_network
                    .setBackgroundResource(R.drawable.image_top_network_focus);
            top_screen_layout
                    .setBackgroundResource(R.drawable.image_top_screen);
            top_screenadjust.setBackgroundResource(Color.TRANSPARENT);
            ...
            top_screenadjust
                    .setBackgroundResource(R.drawable.image_top_screen_focus);
        }
    ...
    void setSharedPrefrences(String name, boolean value) {
        Editor editor = mContext.getSharedPreferences(name,
    ...
    void setOobeStartProp(String value) {
        SystemWriteManager sw = (SystemWriteManager) mContext
